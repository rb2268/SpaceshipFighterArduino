...
Class 5
1/14/22
This program is a videogame that involves a spaceship and has to shoot lasers at enemies
'''
...
def start():
  global startImage, textRectEight, textSurfaceEight
  screen.fill(WHITE)
...
def handleKeyboardEvents():
  ...
  if key[pygame.K_UP] and spaceShipY > 0:
    spaceShipY = spaceShipY - 5
  
  if key[pygame.K_DOWN] and spaceShipY < 240:
    spaceShipY = spaceShipY + 5
  
  if key[pygame.K_LEFT] and spaceShipX > 0:
    spaceShipX = spaceShipX - 5
  
  if key[pygame.K_RIGHT] and spaceShipX < 440:
    spaceShipX = spaceShipX + 5
